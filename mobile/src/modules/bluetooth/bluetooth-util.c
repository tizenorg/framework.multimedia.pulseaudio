...
#ifdef BLUETOOTH_APTX_SUPPORT
    if (aptx_handle)
        register_endpoint(y, path, A2DP_APTX_SOURCE_ENDPOINT, A2DP_SOURCE_UUID, A2DP_CODEC_NON_A2DP);
#endif
    register_endpoint(y, path, A2DP_SOURCE_ENDPOINT, A2DP_SOURCE_UUID, A2DP_CODEC_SBC);
    register_endpoint(y, path, A2DP_SINK_ENDPOINT, A2DP_SINK_UUID, A2DP_CODEC_SBC);
}
...
static pa_bluetooth_transport *transport_new(pa_bluetooth_discovery *y,
                                             const char *path, enum profile p,
                                             uint8_t codec,
...
static DBusMessage *endpoint_set_configuration(DBusConnection *conn, DBusMessage *m, void *userdata) {
    ...
    t = transport_new(y, path, p, codec, config, size);
    if (nrec)
        t->nrec = nrec;
...
#ifdef BLUETOOTH_APTX_SUPPORT
    if (aptx_handle)
        pa_assert_se(dbus_connection_register_object_path(pa_dbus_connection_get(y->connection), A2DP_APTX_SOURCE_ENDPOINT, &vtable_endpoint, y));
#endif
