...
#ifdef USE_SECURITY
    CHECK_VALIDITY(c->pstream, pa_pstream_check_security(c->pstream), tag, PA_ERR_CONNECTIONREFUSED);
#endif /* USE_SECURITY */
    ...
    PA_IDXSET_FOREACH(si, c->protocol->core->sink_inputs, idx) {
        ...
        if (b) {
            ...
            if (pa_sink_input_get_state(si) == PA_SINK_INPUT_RUNNING) {
                ...
                pa_log_info(" <Cork %d> for sink-input [%d][%s]", b, si->index, si->sink->name);
            }
        } else {
            ...
            if ((is_manual_corked_str = pa_proplist_gets(si->proplist, PROP_MANUAL_CORK))) {
                ...
                if (is_manual_corked) {
                    ...
                    pa_log_info(" <UnCork %d> for sink-input [%d][%s]", b, si->index, si->sink->name);
                }
            }
        }
    }
...
static void command_set_default_sink_by_api_bus(pa_pdispatch *pd, uint32_t command, uint32_t tag, pa_tagstruct *t, void *userdata) {
	...
	const char *api, *bus;
	const char *api_string, *bus_string;
	uint32_t idx;
