...
static int suspend(struct userdata *u) {
    ...
    pa_log_warn("Device suspended...[%s]", u->device_name);
...
static int unsuspend(struct userdata *u) {
    ...
    /* FIXME : Need to adjust log level later */
    pa_log_warn("Resumed successfully...[%s]", u->device_name);
...
#ifdef USE_AUDIO_PM
    if (pa_modargs_get_value_boolean(ma, "need_audio_pm", &need_audio_pm) < 0) {
        pa_log("Failed to parse need_audio_pm argument.");
        goto fail;
    }
#endif /* USE_AUDIO_PM */
...
#ifdef USE_AUDIO_PM
    /* Set Need Audio PM property */
	pa_log_info("Set Set Need Audio PM property");
	pa_proplist_setf(data.proplist, "need_audio_pm", "%d", need_audio_pm);
#endif /* USE_AUDIO_PM */
