...
#define USE_CUSTOM_PLAYBACK_BUFFER
#ifdef USE_CUSTOM_PLAYBACK_BUFFER
#define CUSTOM_PERIOD_SIZE 2048
#define CUSTOM_PERIOD_COUNT 4
#endif
...
int pa_alsa_set_hw_params(
    ...
    if (_use_tsched && tsched_size > 0) {
#ifdef USE_CUSTOM_PLAYBACK_BUFFER
        if (snd_pcm_stream(pcm_handle) == SND_PCM_STREAM_PLAYBACK) {
            /* Playback */
            _period_size = CUSTOM_PERIOD_SIZE;
            _buffer_size = _period_size * CUSTOM_PERIOD_COUNT;
        } else {
            /* Capture */
        ...
        _buffer_size = (snd_pcm_uframes_t) (((uint64_t) tsched_size * _ss.rate) / ss->rate);
        _period_size = _buffer_size;
#endif /* USE_CUSTOM_PLAYBACK_BUFFER */
    } else {
    ...
    if (pa_alsa_pcm_is_voip(pcm_handle)) {
        snd_pcm_hw_params_set_period_time(pcm_handle, hwparams, 20000UL, 0);
...
#ifdef PA_USE_QCOM_VOIP
#define VOIP_CAPTURE_WB_STOP_THRESHOLD 3200
#define VOIP_WB_AVAIL_MIN 320
#define VOIP_CAPTURE_NB_STOP_THRESHOLD 1600
#define VOIP_NB_AVAIL_MIN 160
...
int pa_alsa_set_sw_params(snd_pcm_t *pcm, snd_pcm_uframes_t avail_min, pa_bool_t period_event) {
    ...
    if (pa_alsa_pcm_is_voip(pcm)) {
        if (snd_pcm_stream(pcm) == SND_PCM_STREAM_CAPTURE) {
            stop_threshold = (sampling_rate == VOIP_WIDE_BAND) ? VOIP_CAPTURE_WB_STOP_THRESHOLD :VOIP_CAPTURE_NB_STOP_THRESHOLD;
        }
