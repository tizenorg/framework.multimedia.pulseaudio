...
#define YSOUND_CONF			"ysound.conf"
#define DEF_CARD_NAME			"ymu831"
#define CSC_YSOUND_PATH		"/opt/system/csc-default/usr/tuning"
#define DEF_YSOUND_PATH		"/usr/etc/sound"

static char csc_conf_data;
...
/* S-VOICE */
#define STR_WAKEUP_NORMAL					"capture_ap_wakeup_normal"
#define STR_WAKEUP_EARMIC_NORMAL			"capture_ap_wakeup_earmic_normal"
#define STR_WAKEUP_BTMIC_NORMAL				"capture_ap_wakeup_btmic_normal"
#define STR_WAKEUP_DRIVING					"capture_ap_wakeup_driving"
#define STR_SVOICE_NORMAL					"capture_ap_svoice_normal"
#define STR_SVOICE_EARMIC_NORMAL			"capture_ap_svoice_earmic_normal"
#define STR_SVOICE_BTMIC_NORMAL				"capture_ap_svoice_btmic_normal"
#define STR_SVOICE_DRIVING					"capture_ap_svoice_driving"
...
static struct SCENARIO_GROUP gaScnToGrp[] = {
	...
	/* S-VOICE */
	{STR_WAKEUP_NORMAL, "SVOICE_NORMAL"},
	{STR_WAKEUP_EARMIC_NORMAL, "GENERIC_AP"},
	{STR_WAKEUP_BTMIC_NORMAL, "BT_AP"},
	{STR_WAKEUP_DRIVING, "SVOICE_DRIVING"},
	{STR_SVOICE_NORMAL, "SVOICE_NORMAL"},
	{STR_SVOICE_EARMIC_NORMAL, "GENERIC_AP"},
	{STR_SVOICE_BTMIC_NORMAL, "BT_AP"},
	{STR_SVOICE_DRIVING, "SVOICE_DRIVING"},
};
#endif
...
static int check_csc_conf(char *filename)
{
	int ret;
	FILE *f;
...
static int Init(int verbose, char *card)
{
	int nResult;
	char card_name[MAX_PATH_NAME] = {0,};
	...
	if (!card) {
		snprintf(card_name, MAX_PATH_NAME, "%s/%s/%s",
						CSC_YSOUND_PATH, DEF_CARD_NAME, YSOUND_CONF);
	} else {
		snprintf(card_name, MAX_PATH_NAME, "%s/%s/%s",
						CSC_YSOUND_PATH, card, YSOUND_CONF);
	}

	nResult = check_csc_conf(card_name);
	if (!nResult)
		fprintf(stderr, "Use CSC ysound.conf [%d]\n", nResult);
	else
		fprintf(stderr, "Use default ysound.conf [%d]\n", nResult);

	if (!csc_conf_data) {
		if (!card)
		snprintf(card_name, MAX_PATH_NAME, "%s/%s/%s",
					DEF_YSOUND_PATH, DEF_CARD_NAME, YSOUND_CONF);
		else
		snprintf(card_name, MAX_PATH_NAME, "%s/%s/%s",
					DEF_YSOUND_PATH, card, YSOUND_CONF);
	}

	nResult = ReadFile(card_name, &gsFileData);
	if (nResult < 0) {
		free(gpGroupInfo);
		gpGroupInfo = NULL;
		return nResult;
...
void print_groups(char *cardname)
{
	 if (Init(1, cardname) < 0) {
