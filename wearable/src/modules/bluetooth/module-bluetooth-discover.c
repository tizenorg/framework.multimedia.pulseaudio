...
#ifdef __TIZEN_BT__
            if (d->profile_state[PROFILE_A2DP] >= PA_BT_AUDIO_STATE_CONNECTED)
...
#endif
#ifdef BLUETOOTH_SCMST_SUPPORT
    const pa_bool_t enable_scmst = FALSE;
#endif
    pa_assert(m);
...
#ifdef BLUETOOTH_SCMST_SUPPORT
    if (pa_modargs_get_value_boolean(ma, "enable_scmst", &enable_scmst))
	pa_log("Failed to parse enable_scmst argument");
    else
	pa_set_scmst_status(enable_scmst);
#endif
