...
static void bt_transport_config_mtu(struct userdata *u) {
	...
	if (a2dp ->sbc_initialized) {
		pa_log_debug("Caluculate MTU for SBC codec");
		u->read_block_size =
				(u->read_link_mtu - sizeof(struct rtp_header) - sizeof(struct rtp_payload))
...
#ifdef BLUETOOTH_SCMST_SUPPORT
    uint8_t *ptr;
    pa_bool_t scmst_enabled = pa_get_scmst_status();
    pa_bool_t bt_cp_status = pa_get_a2dp_cp_status();
    pa_bool_t app_cp_status = pa_get_app_cp_status();
#endif
    int ret = 0;
...
#ifdef BLUETOOTH_SCMST_SUPPORT
    if (bt_cp_status && scmst_enabled) {
	d = (uint8_t*) a2dp->buffer + sizeof(*header) + sizeof(*payload) + 1;
	to_write = a2dp->buffer_size - sizeof(*header) - sizeof(*payload) - 1;
    } else {
...
#ifdef BLUETOOTH_SCMST_SUPPORT
   if (bt_cp_status && scmst_enabled) {
   /*Based on the applicatoin requirement Copyu protection shall be added */
        ptr = (uint8_t*)a2dp->buffer + sizeof(*header) +  sizeof(*payload);
	if(app_cp_status)
	    *ptr = SCMS_T_COPY_NOT_ALLOWED;
	else
	    *ptr = SCMS_T_COPY_ALLOWED;
    }
#endif
    header->v = 2;
    header->pt = 1;
...
#ifdef BLUETOOTH_SCMST_SUPPORT
	if (bt_cp_status == FALSE && app_cp_status == TRUE)
	    l = nbytes;
	else
