...
#ifdef BLUETOOTH_SCMST_SUPPORT
#define CP_TYPE_LSB 0x02
#define CP_TYPE_MSB 0x00

#define BLUETOOTH_CP_OBJ_PATH "/org/tizen/bluetooth/a2dpcontentprotection"

static pa_bool_t enable_scmst = FALSE;

static pa_bool_t bt_a2dp_cp_status;
static pa_bool_t app_cp_status;

pa_bool_t pa_set_scmst_status(pa_bool_t scmst_status)
{
	enable_scmst = scmst_status;
}

pa_bool_t pa_get_scmst_status(void)
{
	return enable_scmst;
}

pa_bool_t pa_get_app_cp_status(void)
{
	return app_cp_status;
}

pa_bool_t pa_get_a2dp_cp_status(void)
{
	return bt_a2dp_cp_status;
}

static void  pa_set_a2dp_cp_status(pa_bool_t status)
{
	bt_a2dp_cp_status = status;
}
#endif
...
#else
                    if (strcasecmp(A2DP_SINK_UUID, value) == 0) {
                        pa_assert_se(m = dbus_message_new_method_call("org.bluez", d->path, "org.bluez.AudioSink",
                                                                      "GetProperties"));
                        send_and_add_to_pending(d->discovery, m, get_properties_reply, d);
                        has_audio = true;
...
#ifdef BLUETOOTH_SCMST_SUPPORT
		if (state == PA_BT_AUDIO_STATE_DISCONNECTED)
		   pa_set_a2dp_cp_status(FALSE);
#endif
...
#ifdef BLUETOOTH_SCMST_SUPPORT
	case DBUS_TYPE_BOOLEAN: {
		if (pa_streq(key, "Protected")) {
			pa_bool_t value;
			dbus_message_iter_get_basic(&variant_i, &value);
			pa_log_debug("dbus: property 'Protected' changed to value '%d'", value);
			pa_set_a2dp_cp_status(value);
		}
	}
	break;
#endif
...
static void register_endpoint(pa_bluetooth_discovery *y, const char *path, const char *endpoint, const char *uuid, unsigned codec) {
    DBusMessage *m;
    DBusMessageIter i, d;
...
#ifdef BLUETOOTH_SCMST_SUPPORT
			uint8_t cp_cap[] = { CP_TYPE_LSB, CP_TYPE_MSB };
#endif
...
#ifdef BLUETOOTH_SCMST_SUPPORT
			if (enable_scmst)
				pa_dbus_append_basic_array_variant_dict_entry(&d, "ContentProtection", DBUS_TYPE_BYTE, &cp_cap, sizeof(cp_cap));
#endif
		}
#ifdef BLUETOOTH_APTX_SUPPORT
...
static void found_adapter(pa_bluetooth_discovery *y, const char *path) {
 ...
 #ifndef __TIZEN_BT__
    register_endpoint(y, path, HFP_AG_ENDPOINT, HFP_AG_UUID, 0);
    register_endpoint(y, path, HFP_HS_ENDPOINT, HFP_HS_UUID, 0);
#endif
    register_endpoint(y, path, A2DP_SOURCE_ENDPOINT, A2DP_SOURCE_UUID, 0);
#ifdef BLUETOOTH_APTX_SUPPORT
    if (aptx_handle)
	register_endpoint(y, path, A2DP_APTX_SOURCE_ENDPOINT, A2DP_SOURCE_UUID, A2DP_CODEC_VENDOR);
#endif

 #ifndef __TIZEN_BT__
    register_endpoint(y, path, A2DP_SINK_ENDPOINT, A2DP_SINK_UUID, 0);
 #endif
}
...
#ifdef  BLUETOOTH_SCMST_SUPPORT
   else if (dbus_message_is_signal(m, "org.tizen.bluetooth.A2dpContentProtection", "ProtectionRequired")) {
		pa_bool_t status;
		const char *path;

		path = dbus_message_get_path(m);
		if (path == NULL || !pa_streq(path, BLUETOOTH_CP_OBJ_PATH ))
			goto fail;

		if (!dbus_message_get_args(m, &err, DBUS_TYPE_BOOLEAN, &status, DBUS_TYPE_INVALID)) {
			pa_log("Failed to parse org.tizen.bluetooth.A2dpContentProtection: %s", err.message);
			goto fail;
		}
		app_cp_status = status;
		pa_log_debug("app content protection status = [%d] \n", app_cp_status);
...
static pa_bluetooth_transport *transport_new(pa_bluetooth_device *d, const char *owner, const char *path, enum profile p,
                                             uint8_t codec, const uint8_t *config, int size) {
    pa_bluetooth_transport *t;
...
static DBusMessage *endpoint_set_configuration(DBusConnection *conn, DBusMessage *m, void *userdata) {
    ...
    t = transport_new(d, sender, path, p, codec, config, size);
    if (nrec)
        t->nrec = nrec;
...
static DBusHandlerResult endpoint_handler(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if (!pa_streq(path, A2DP_SOURCE_ENDPOINT) && !pa_streq(path, A2DP_SINK_ENDPOINT) && !pa_streq(path, HFP_AG_ENDPOINT) &&
        !pa_streq(path, HFP_HS_ENDPOINT) && !pa_streq(path,A2DP_APTX_SOURCE_ENDPOINT))
#else
    if (!pa_streq(path, A2DP_SOURCE_ENDPOINT) && !pa_streq(path, A2DP_SINK_ENDPOINT) && !pa_streq(path, HFP_AG_ENDPOINT) &&
...
#endif
#ifdef BLUETOOTH_SCMST_SUPPORT
		"type='signal',interface='org.tizen.bluetooth.A2dpContentProtection',member='ProtectionRequired'",
#endif
                NULL) < 0) {
